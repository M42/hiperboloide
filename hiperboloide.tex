...
% Resumen
\newcommand{\docabstract}{Este trabajo presenta algunos modelos de la
  geometría hiperbólica y varias construcciones y teselaciones posibles
  sobre ellos. Finaliza repasando el software disponible para la manipulación
  interactiva del plano hiperbólico.
}
...
Lobachevsky. A pesar de este cambio, los aspectos y teoremas
de la geometría euclídea que sean independientes del quinto postulado
seguirán siendo válidos.

Históricamente, se realizaron esfuerzos por deducir el quinto
...
\begin{enumerate}
\item \textbf{vectores espaciales}, $x\in\mathbb{R}^3$ tales que $L(x)>0$;
\item \textbf{vectores temporales}, $x\in\mathbb{R}^3$ tales que $L(x)<0$; y
\item \textbf{vectores luminosos}, $x\in\mathbb{R}^3$ tales que $L(x)=0$.
\end{enumerate}
...
Así, utilizando la relación con el proyectivo, por cualesquiera dos
puntos de $\mathbb{H}^2$ pasará una única recta, la correspodiente a la
que nos da la geometría proyectiva; si dos rectas se intersectan es en
un único punto y las rectas se extienden indefinidamente en dos
direcciones.

Vamos a ver que efectivamente no se cumple el quinto postulado. Sea
$r$ una recta hiperbólica, y $p$ un punto fuera de ella. Abusando de
notación, podemos llamar $r$ también a la recta proyectiva
correspondiente. Por cada punto $q\in{r}$ que no sea temporal podemos
trazar la recta que pasa por $p$ y $q$, la denotamos por $s_q$. Puesto
que $q\in{s_q}$ esta representa un recta hiperbólica que no intersecta
a $r$ en $\mathbb{H}^2$, puesto que
$r\cap{s_q}=q\not\in\mathbb{H}^2$. Así que tenemos varias rectas que
no cortan a nuestra recta inicial $r$,luego el quinto postulado no se
cumple en su parte de unicidad.

Clasificamos dos rectas según su intersección en

\begin{enumerate} 
\item{Rectas concurrentes: Si la intersección es un punto temporal.}
\item{Rectas paralelas: Si la intersección es un punto de luz.}
\item{Rectas ultraparalelas: Si la intersección es un punto espacial.}
\end{enumerate}
...
La proyección de $\mathbb{R}^3$ menos el origen en el plano proyectivo
$\mathbb{P}^2$ lleva el cono de luz, el cono formado por los vectores
luminosos, a la circunferencia $S_1$ del plano proyectivo en el plano
$z=1$.  El plano hiperbólico se proyecta de manera biyectiva sobre los
puntos de su interior (pues una recta temporal, es decir, formada por
vectores temporales, intersecta a $\mathbb{H}_2$ en un único
punto). En general, cualquier recta no luminosa tiene exactamente dos
vectores unitarios con respecto a la norma definida por la métrica de
Lorentz que son inversos con respecto a la suma igual que pasa con la
métrica euclídea. Si la recta está formada por vectores temporales,
entonces uno de ellos está en $\mathbb{H}_2$.

Podemos entonces identificar a $\mathbb{H}_2$ con el interior del
disco unitario, y a esos puntos llamarlos \textit{puntos del plano
hiperbólico}. Los puntos luz son los del círculo $S_1$ que no están en
el hiperbólico. A los demás puntos, fuera del disco unidad, se les
llamará \textit{puntos espaciales}. Este es el modelo proyectivo que es muy
conveniente para hacer dibujos.

La definición de rectas hiperbólicas se explica mejor ahora, ya que son
las intersecciones de rectas proyectivas con el plano hiperbólico.

La proyección anterior puede realizarse de la siguiente
manera: dado un punto $(t, x_1,x_2)$ sobre la hoja superior de un
hiperboloide del modelo del hiperboloide, se define un punto del
...
\subsection{Circunferencias, horociclos e hiperciclos}
...
\begin{definition}[Circunferencia]
  Dado un punto $C$ y un radio $r \geq 0$, llamamos \textbf{circunferencia}
